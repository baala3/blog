...
In general, every OAuth client needs to be manually registered with an authorization server before it can request tokens. Usually, this means an admin opens a portal, types in a name and redirect URI, copies the client_id and client_secret into a config file, and hits deploy. For a few internal apps, that’s works. But that manual flow breaks completely when you want to register thousands of clients. This whre Dynamic Client Registration (DCR) enters.

## The Specs: RFC 7591 and 7592

To solve the scaling problem, we have two key standards:
...
- [RFC 7592](https://datatracker.ietf.org/doc/html/rfc7592): Extends the protocol with management API to read, update, and delete those registrations.

In this post, we’ll dive into how these specs work and walk through working Ruby implementation I have built on [Roda](https://roda.jeremyevans.net/) and [rodauth-oauth](https://github.com/HoneyryderChuck/rodauth-oauth).
...
- **No Lifecycle Standard**: Every provider has a different UI or proprietary API. If your client needs to rotate its secret, update its redirect URIs, or delete itself when a customer churns, you’re stuck writing ugly code for every single provider.
...
The client sends a JSON body with its metadata, and the server responds with `client_id` and `client_secret` (if applicable). The response code is `201 Created`, not `200 OK`.
...
- The `registration_client_uri` and `registration_access_token` are only returned if the server supports RFC 7592 (the management spec). These are your keys to manage the client later.
- The `Authorization: Bearer` header contains the initial access token. This is pre-shared credential that determines who is allowed to register. Without this, your endpoint is open to entire internet—and you'll be drowning in "garbage" registrations and DOS attacks.
- DCR introduces `software_id` and `software_version` to identify the "version" of the app, distinct from the specific instance (client_id). For high-security environments, `software_statement` (a signed JWT) allows the server to cryptographically verify exactly what software is knocking at the door.
...
>note: The server may rotate the `registration_access_token` on every read response, if it does (and returns key in the response), the old token must be discarded.
...
**PUT is a full replacement, not a patch.** The spec is explicit that: "valid values of client metadata fields in this request MUST replace, not augment, the values previously associated with this client." If you omit `redirect_uris`, they're gone so send all fields every time.
...
To see this in action, I’ve put together a sample application: [roda-oauth-dynamic-registration](https://github.com/baala3/roda-oauth-dynamic-registration). It implements both RFCs using the [Roda](https://roda.jeremyevans.net/) web toolkit and [rodauth-oauth](https://github.com/HoneyryderChuck/rodauth-oauth) in just about 100 lines of code.

If you want to get your hands dirty and test the registration flow yourself, you can check out the repository and follow the setup guide in the README.
