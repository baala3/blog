...
- **Part 1 (this post):** The framework, assurance levels, and risk management model
...
Most organizations approach identity security as a compliance exercise. You implement MFA because your auditor requires it. You add SSO because it's on the enterprise checklist. The security properties you actually get are an afterthought.
...
The spec is organized as a suite of four documents. This post covers the main volume (SP 800-63-4), which defines the model and risk management process. The companion volumes cover identity proofing (63A), authentication (63B), and federation (63C).

---

# The Digital Identity Model

Before getting to assurance levels, it helps to understand who the spec thinks is in the room.

## The Cast

**Applicant:** Someone seeking access to a digital service. Before they're enrolled, they're an applicant.
...
**Non-federated:** The CSP, IdP, and RP are the same system. This is the classic "your app has a login page." The user registers, your app stores their credentials, your app authenticates them. Simple, but every RP has to manage the full identity lifecycle.

```
Applicant → [CSP/IdP/RP] → Subscriber Session
```

**General-purpose federation:** The CSP provisions a subscriber account; the IdP handles authentication and issues assertions to separate RPs. This is how enterprise SSO and consumer identity providers (Sign in with Google, GitHub OAuth) work.

```
Subscriber ──auth──► IdP ──assertion──► RP
                      ▲
                      │
                     CSP (provisioned the account)
```

**Subscriber-controlled wallet:** New in 800-63-4. The CSP issues signed attribute bundles (verifiable credentials) directly to the subscriber. The subscriber's wallet manages them and presents them directly to RPs, without the IdP being involved at assertion time.

```
CSP ──issues credentials──► Subscriber Wallet
                                    │
                             presents to RP
                                    │
                                    ▼
                                   RP (verifies CSP signature directly)
```
...
This is the core idea of the framework. Identity assurance is not one thing. It's three independent properties, each of which can be at a different level depending on what your service needs.
...
This is the part that trips people up. IAL, AAL, and FAL measure different things. You can have any combination:
...
The framework doesn't just hand you a table of levels and say "pick one." It describes a five-step process for deciding which levels are right for your service.
...
Based on the impact assessment, select a baseline IAL, AAL, and FAL. The spec provides guidance on which impact levels correspond to which assurance levels.

## Step 4: Tailor Controls and Document Decisions

The initial xAL selection is a starting point. You can go higher or lower with documented justification. If your baseline analysis suggests IAL2 but the user population includes people who can't provide standard government ID (elderly users, recent immigrants, people experiencing homelessness), you might add trusted referee processes rather than simply requiring IAL2 and excluding those users.
...
The value of 800-63-4 isn't that it tells you exactly what to build. It's that it gives you a shared vocabulary and a structured way to think about trade-offs. When someone says "we need stronger auth," the framework helps you ask: stronger in what way? For which users? Against which threats? With what impact if it fails?
...
Posts 2 through 4 go deeper into each volume: identity proofing in 63A, authentication in 63B, and federation in 63C. Each one has its own set of requirements worth understanding in detail.
